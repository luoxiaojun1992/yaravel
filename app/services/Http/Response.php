...
class Response implements ResponseInterface
{
    ...
    public function __construct($status = 200, array $headers = [], $body = null, $version = '1.1', $reason = null)
    {
        ...
        if (\Registry::has('context.http.request')) {
            ...
            $httpVersionFlag = 'HTTP/';
            if (Str::startsWith($protocolVersion, $httpVersionFlag)) {
                $httpVersion = substr($protocolVersion, strlen($httpVersionFlag));
                if (is_numeric($httpVersion)) {
                    $this->version = $version = $httpVersion;
                }
            }
        }
        ...
        \Registry::alias('context.http.response', $this);
    }
    ...
    public function setCookie($name, $value = "", $expire = 0, $path = "", $domain = "", $secure = false, $httpOnly = false)
    {
        if ($this->yafResponse) {
            ...
            if ($expire > 0) {
                $cookieParts[] = 'expires=' .
                    gmstrftime("%A, %d-%b-%Y %H:%M:%S GMT",time() + $expire);
            }
            if ($path) {
    ...
    public function sendHeaders()
    {
        ...
        // status
        if ($this->yafResponse) {
            $this->yafResponse->setHeader(
                sprintf('HTTP/%s %s %s', $this->version, $this->statusCode, $this->getReasonPhrase()),
                null,
                true,
                $this->statusCode
            );
